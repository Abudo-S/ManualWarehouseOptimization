...
class MultiCriteriaRecGNNModel(torch.nn.Module):
    def __init__(self, 
        ...
        #GRU updates each operator's hidden state. 
        ...
        #activation head
        #RNN hidden state (64) + global_u (3) + demand (1) + monotonic_id (1) + op_pe (8) = 77
        ...
        #assignment head
        #RNN hidden state (64) + order emb (64) + global_u (3) + time (2) + act_prob (1) + monotonic_id (1) + op_pe (8) + affinity_score (1) = 144
        input_dim_assignment = (2 * hidden_dim) + 3 + 2 + 1 + 1 + 8 + 1
    ...
    def decode_step(self, static_embs, x_dict_raw, edge_index_dict, edge_attr_dict, u, batch_dict, op_hidden=None, last_order_emb=None):
        ...
        assign_input = torch.cat([
            op_emb_assign, ord_emb_assign, u_edges, edge_attr, 
            op_activation_prob, src_monotonic_id, op_pe[src_idx], affinity_score
        ], dim=1)
