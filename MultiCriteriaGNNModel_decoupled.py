...
SAVE_MODEL_IN_TRAINING_PATH = 'checkpoints/gnn_checkpoint_epoch_{idx}.pth' # replace idx with epoch number when saving

class MultiCriteriaGNNModel(torch.nn.Module):
    def __init__(self, metadata, hidden_dim=64, num_layers=3, heads=4, dropout=0.2, heuristic_boost_factor=1.15):
        """
        ...
        [0,1] range before being fed into the model for better training stability. This can be done by dividing the raw time 
        values by a constant (e.g., 480.0 minutes for an 8-hour shift) to bring them to a similar scale as the node features.
        """
        super().__init__()
        ...
        for _ in range(num_layers):
            conv_dict = {
                ...
                ('operator', 'assign', 'order'): GATv2Conv(
                    ...
                    hidden_dim // heads, 
                    heads=heads, 
                    edge_dim=2, # edge_dim=2 for processing time + travel time from the base
                    add_self_loops=False, 
                    dropout=dropout
                ),
        ...
        #input: op_emb(64) + ord_emb(64) + global(3) + edge_attr(2) [time, base_travel_time] = 132
        self.assign_head = Sequential(
            Linear((2 * hidden_dim) + 3 + 2, hidden_dim),
            ReLU(),
            Linear(hidden_dim, 1)
        )

        #sequence head (edge classification for order -> order)
        #input: ord_emb_i(64) + ord_emb_j(64) + global(3) + edge_attr_time(1) = 132
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict, u, batch_dict=None):
        ...
        #mix original message-passed embeddings (preserve identity while adding context)
        ...
        op_demand_feature = load_ratio_per_batch[batch_dict['operator']] #normalized [0,1]
        ...
        #DECOUPLED: No probabilities from activation head are concatenated
        ...
        edge_attr = edge_attr_dict[('order', 'to', 'order')] 
        ...
        edge_batch_indices = ord_batch[src_idx]
        u_edges = u[edge_batch_indices]

        #DECOUPLED: No shared_op_scores or active_shared_scores are concatenated
